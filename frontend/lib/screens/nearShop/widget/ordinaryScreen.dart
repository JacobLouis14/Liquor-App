...
class OrdinaryScreen extends StatefulWidget {
  const OrdinaryScreen({super.key});
  @override
  State<OrdinaryScreen> createState() => _OrdinaryScreenState();
}

class _OrdinaryScreenState extends State<OrdinaryScreen> {
  List<ShopModel>? ordinaryShopList;

  void ordinaryShopListCheckUp() async {
    //Checking Cordinates available
    await geoLocationAccess(context);
    setState(() {
      ordinaryShopList = shopListData!
          .where((element) => element.category!.toLowerCase() == "ordinary")
          .toList();
      //Adding Arieal Distance Property Value
      if (currentLatitudeGlobalData != null &&
          currentLongitudeGlobalData != null) {
        ordinaryShopList = ordinaryShopList!.map((shop) {
          shop.ariealDistance =
  ...
  @override
  void initState() {
    ordinaryShopListCheckUp();
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    if (ordinaryShopList != null) {
      return Container(
          padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 30),
          child: ordinaryShopList!.isNotEmpty
              ? Column(
                  children: ordinaryShopList!.map((shop) {
                    return ListTile(
                      onTap: () {
                        if (currentLatitudeGlobalData != null &&
                            currentLongitudeGlobalData != null) {
